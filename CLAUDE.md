...
- `Storage:AzureBlob:Url` / `Storage:AzureBlob:AccountName` / `Storage:AzureBlob:AccessKey` / `Storage:AzureBlob:ContainerName` — where `AzureBlobIntegration` drops the generated Instagram image (`.png`) and caption (`.txt`). The container is `instagram-generated-posts` and must stay **private**: the image is exposed to Meta only through a short-lived SAS link.
...
The post is then published through `Integration/InstagramIntegration.cs`, which follows the Graph API's three steps: `POST /{ig-user-id}/media` (image URL + caption) returns a `creation_id`; `GET /{creation_id}?fields=status_code,status` is polled until `FINISHED` (up to 10 attempts, 2s apart — the container downloads the image asynchronously, and publishing early fails); then `POST /{ig-user-id}/media_publish` publishes it. Any `status_code` other than `FINISHED`/`IN_PROGRESS` (`ERROR`, `EXPIRED`) aborts with the `status` text in the exception.

Credentials come from the customer's own `CustomerPlatformConfiguration` — `InstagramAccessToken` (stored **in clear text**, unlike `Password`) and `InstagramUserId` — and travel in the request body, so `IRestClient` has `post(url, data)` / `get(url)` methods with no Basic auth that surface the response body in the `RestException` (that's where Graph describes its errors). The versioned Graph API base URL is pinned in `Constants.INSTAGRAM_GRAPH_API_URL`, alongside the model names — it tracks Meta's version deprecation cycle, not the environment. Since the blob container is private, `AzureBlobIntegration.GenerateTemporaryReadUrl` signs a 1-hour read-only SAS URL for Meta to fetch the image; the persisted `ContentUrl` remains the plain blob URL, and `ContentSummary` the image description. The `Content` row is only written after a successful publish.
